...
diff_checker <- function(fulldat, dis, outcome, potouts, resdis){
  ...
  if(nrow(both) < 1 | nrow(neither) < length(disfacs)){return(FALSE)} else {
    ...
    if (!out & length(confs) != length(noconfs)){
      ...
      dissuff <- paste0(dislong, "->", outcome)
      discon <- condition(dissuff, fulldat)
      discon <- attributes(discon)$info$consistency
      ...
      confcons <- lapply(confsuff, function(x) condition(x, fulldat))
      confcons <- unlist(lapply(confcons, function(x) attributes(x)$info$consistency))
